...
import { scopeFor, type Principal, type RlsConfig } from "./rls.ts";
export class ScopeError extends Error { status = 403; }
export function requireScope(model: Model, base: string, cfg: RlsConfig, principal: Principal | null) {
  const scope = scopeFor(model, base, cfg, principal);
...
export function scopedField(model: Model, conn: Connector, base: string, field: string, cfg: RlsConfig, principal: Principal | null) {
  ...
  const scope = requireScope(model, base, cfg, principal);
